...
type [<AllowNullLiteral>] [<MenuAttribute("Улицы", PermissionNamesForFodels.ViewModels, Sections.Models, "Street", Icons.Star)>] Street() =
    member this.Id
        with get () = this.id
    ...
    private new(lazyLoader: ILazyLoader) as newStreet =
        Street()
        then
            newStreet.LazyLoader <- lazyLoader
